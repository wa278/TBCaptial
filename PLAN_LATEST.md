...
| `scripts/install_akquant_backend.sh` | 是 | 校验 AKQuant tag/commit，固定构建时间，构建/安装 wheel 并输出 SHA-256 |
...
本章接口和实现边界参考本机 `/Users/wa/akquant` 仓库，重点检查了 `run_backtest`、`Strategy`、`StrategyContext`、`DataFeedAdapter`、`BacktestResult`、目标权重示例、T+1、多标的一致性、自定义撮合器和公司行为实现。检查基线为干净 tag `v0.3.2`、commit `2924e0cff36669a3563ffb5cb139da0ba9254045`，许可证为 MIT。
...
1. **已关闭（BT-000）**：AKQuant 固定为 `v0.3.2` commit `2924e0cff36669a3563ffb5cb139da0ba9254045`；安装脚本拒绝未知/脏源码，以 commit 时间固定 `SOURCE_DATE_EPOCH`，当前 `osx-64` 连续构建 wheel hash 一致，并验证导入和最小真实回测。wheel 仍按目标平台本地构建，不把机器绝对路径写进环境声明。
