...
- `download_akshare_data.py`：下载交易日历、P0 股票日线和基准，保留 Raw、请求信息和 hash；
  东财失败时按参数熔断到新浪。
- `download_akshare_complete.sh`：现有本地股票的公司、公司行为和财务历史扩展入口；自动激活环境。
- `download_akshare_complete.py`：可续跑下载公司概况、IPO、分红、股本/行业变更、东财主要财务
  指标与主营构成，以及新浪三大完整财务报表；单任务失败不丢弃其他已落盘 Raw，并在
  enrichment manifest 中显式记录失败。
- `preview_akshare_data.sh`：数据预览的一条命令入口；自动激活环境并传递筛选参数。
- `preview_akshare_data.py`：按明确 download/enrichment manifest 只读预览 Parquet，支持数据集、
  股票、头尾和行数筛选，不跨 Raw 批次隐式拼接。
- `summarize_akshare_data.sh`：本地 AKShare Raw 统计的一条命令入口；自动激活环境。
- `summarize_akshare_data.py`：只读汇总所有 Raw 批次、完成任务、有效股票日期行和孤立批次，
  可输出供自动化使用的 JSON。
...
- `run_factor_backtests.sh`：三因子本地研究回测入口；自动激活环境并原样传递参数。
- `run_factor_backtests.py`：校验并对齐明确的 Raw download manifest，运行三个固定
  因子策略，原子发布表格、决策记录、run manifest 和离线 Plotly 报告。
...
- `research/raw_data.py`：严格校验单个 Raw download manifest 和其文件 hash，对齐股票与基准
  OHLCV；该路径是临时研究输入，不得称为 snapshot。
- `research/strategies.py`：三个仅依赖 TBCaptial `BarSliceStrategy` 端口的只做多因子策略。
- `research/suite.py`：统一调用 AKQuant adapter 并计算简化的净值、回撤、基准和绩效摘要。
- `research/reporting.py`：在本地 `var/runs/factors/` 原子发布 CSV/Parquet/JSON 审计产物与
  内嵌 Plotly 的离线 HTML 报告。
...
- `tests/data/test_akshare_scripts.py`：离线验证东财失败后的任务级熔断、新浪 fallback、Raw manifest
  和下载校验器。
- `tests/research/test_factor_research.py`：用合成 Raw manifest 离线验证文件篡改/OHLCV 失败、
  对齐、三个真实 AKQuant 因子回测及完整离线报告产物。
...
- `var/runs/`：本地回测运行产物；三因子研究产物位于 `var/runs/factors/<run_id>/`。
...
make factor-backtests \
  FACTOR_BACKTEST_ARGS="--manifest var/data/manifests/downloads/<run_id>.json"
```

三因子工作流是 Raw 数据上的研究和工程验证，必须保留以下警告：价格未复权，manifest
列出的当前样本不是历史全市场股票池，存在公司行为失真、幸存者/选样偏差和行业集中。它不能
替代 Silver 质量门和正式 snapshot，产物只供研究验证，不得用于投资决策。
