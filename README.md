...
> 当前状态：**环境与 AKQuant backend 基线已验收**。Conda、AKShare Raw 小切片脚本、固定 AKQuant `v0.3.2` wheel、最小回测 adapter 和离线契约测试已经落地，并提供一条基于已校验 Raw download manifest 的三因子研究验证工作流；Silver、正式 snapshot 与完整回测产物链路仍待实现。
...
当地 `var/data` 已有至少 3 只股票、每只至少 120 条日线的完成下载清单时，可一条命令运行三个只做多的因子示例：

```bash
make factor-backtests
```

为了让输入边界更明确，推荐研究记录使用指定 Raw download manifest：
...
- `momentum_60d`：60 交易日横截面动量，每 20 日调仓，仅持有动量为正的最强标的；
- `reversal_5d`：5 交易日短期反转，每 5 日调仓，仅持有近期下跌最多的标的；
- `low_volatility_20d`：按 20 交易日实现波动率的倒数分配权重，每 20 日调仓。

三个策略均在 T 日收盘后计算因子和目标权重，由固定 AKQuant backend 在下一事件开盘尝试成交，并启用 T+1、100 股整手和显式费用参数。因 next-event 风控拒绝的换仓目标会在下一个收盘重申一次，报告仍按实际成交计算并单列拒单和期末未完成订单。默认从 2023-08-28 开始，以匹配当前固定的 0.05% 卖出印花税参数。运行结果写入 `var/runs/factors/<run_id>/`，包含可完全离线打开的 Plotly `report.html`、CSV/Parquet 指标与净值数据、各策略的持仓/订单/成交/决策记录，以及带输入和产物 hash 的 `manifest.json`。

> 这是研究与工程链路验证，不是正式投资回测。当前输入是单个 Raw download manifest，价格未复权，其中列出的当前样本并非历史全市场股票池；无论 manifest 有 3 只还是更多股票，仍存在分红/拆分失真、幸存者偏差、选样偏差和行业集中。在 Silver 标准化、正式 snapshot、历史股票池、公司行为和交易状态数据完成前，不得将收益指标用于策略选择或实盘决策。详细命令和产物定义见 [OPERATIONS.md](./OPERATIONS.md)。
...
- `scripts/init_submodules.sh`：从 `.gitmodules` 初始化 AKQuant 并校验 remote、commit 和干净工作区；
- `scripts/install_akquant_backend.sh`：校验固定 submodule commit，以 commit 时间构建可重复 wheel，安装并输出 SHA-256；
...
AKQuant 通过 `third_party/akquant` Git submodule 绑定到 `git@github.com:wa278/akquant.git` 的 commit `2924e0cff36669a3563ffb5cb139da0ba9254045`，不依赖 tag；构建后包版本必须为 `0.3.2`。`environment.yml` 声明其运行与 Rust/Maturin 构建依赖，`make setup` 初始化 submodule 并构建 release wheel。本机 `osx-64` 连续构建已得到一致 wheel SHA-256，具体记录见 `ENVIRONMENT_ACCEPTANCE.md`。
